...
## 触发条件
- 收到 `self_study` 消息（daemon 定期注入，消息含 `instruction` 字段）
...
2. **恢复检查**：如果有状态为 `learning` 且添加时间超过 24h 的课题，重置为 `pending`
3. 选优先级最高的 `pending` 课题
4. 更新状态 `pending → learning`
5. 如果没有 pending 课题 → 结束
...
2. 更新 `learning-queue.md`：`learning → done`，移到"已完成"表
3. 如果第 2 思建议新建 Skill → 通知中明确提出，等用户确认
